...
const isNarrowChart = () => emotionChartRef.value?.clientWidth < 500;
...
const renderEmotionChart = () => {
  ...
  emotionChart.setOption({
    ...
    xAxis: {
      ...
      axisLabel: { rotate: narrow ? 30 : 0, fontSize: narrow ? 9 : 11 }
    },
    yAxis: [
      { type: 'value', name: narrow ? '' : '评分', min: 0, max: 10, axisLabel: { fontSize: narrow ? 9 : 11 } },
      { type: 'value', name: narrow ? '' : '数量', axisLabel: { fontSize: narrow ? 9 : 11 } }
    ],
    series: [
...
const renderConsultationChart = () => {
  ...
  consultationChart.setOption({
    ...
    xAxis: {
      type: 'category',
      data: consultationTrend.value.map((item) => item.date),
      axisLabel: { rotate: narrow ? 30 : 0, fontSize: narrow ? 9 : 11 }
    },
    yAxis: { type: 'value', name: narrow ? '' : '数量', axisLabel: { fontSize: narrow ? 9 : 11 } },
    series: [
...
const renderUserActivityChart = () => {
  ...
  userActivityChart.setOption({
    ...
    xAxis: {
      type: 'category',
      data: userActivityTrend.value.map((item) => item.date),
      axisLabel: { rotate: narrow ? 30 : 0, fontSize: narrow ? 9 : 11 }
    },
    yAxis: { type: 'value', name: narrow ? '' : '用户数', axisLabel: { fontSize: narrow ? 9 : 11 } },
    series: [
